...
export const editAnimal = async (req: Request, res: Response) => {
    try {
        ...
        // Create animal
        const animal = await animalClient.update({
            where: {
                id: id,
                identifier: identifier,
                laboratoryId: laboratory.id
        ...
        if (createdAnimalType) {
            ...
            response.message = 'Animal and animal type updated successfully';
        } else if (usedExistingType) {
            response.message = 'Animal updated successfully using existing animal type';
        }
